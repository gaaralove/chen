...
import { 
  ChefHat, Cpu, Zap, Box, 
  Settings, User, X, Plus, CheckCircle2, MapPin, TrendingUp, Globe, ArrowUpRight, ArrowDownRight, Minus, Sparkles, Utensils, History, ChevronRight, Mic
} from 'lucide-react';
...
const BUILTIN_PLUGINS: AssistantPlugin[] = [FoodDeliveryPlugin, SystemControlPlugin];
...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F8FAFC] font-sans text-slate-900 max-w-md mx-auto border-x border-slate-200 shadow-2xl overflow-hidden relative">
      ...
      <header className="px-8 pt-8 pb-6 flex items-center justify-between z-20 shrink-0">
        ...
        <div className="flex gap-3">
           <button onClick={() => setShowPanel('docs')} className="p-4 bg-white shadow-sm rounded-2xl text-slate-400 border border-slate-100 active:scale-90 transition-transform">
             <Plus size={20} />
           </button>
           <button onClick={() => setShowPanel('settings')} className="p-4 bg-white shadow-sm rounded-2xl text-slate-400 border border-slate-100 active:scale-90 transition-transform">
             <Settings size={22} />
           </button>
        </div>
      </header>
      ...
      <main className="flex-1 relative z-10 overflow-hidden flex flex-col px-8">
        {showPanel ? (
          <div className="pb-6 flex-1 overflow-hidden animate-in slide-in-from-right duration-400">
            <div className="w-full h-full bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden relative">
               ...
               {showPanel === 'docs' ? <ExpansionDoc /> : 
                ...
                showPanel === 'profile' ? <div className="p-10 text-center"><TrendingUp size={64} className="mx-auto text-slate-100 mb-8"/><p className="text-sm text-slate-400 font-black uppercase tracking-widest">神经画像深度解析中...</p></div> : null}
            </div>
          </div>
        ) : (
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex-1 flex flex-col items-center justify-center min-h-0 relative">
               ...
               <div className={`w-full transition-all duration-500 ease-in-out flex flex-col items-center justify-center ${pluginData ? 'h-full py-4' : 'h-1/3 opacity-100'}`}>
                  {lastTranscript && !pluginData && (
                    <div className="bg-white/90 backdrop-blur-2xl px-8 py-6 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-white text-center animate-in zoom-in max-w-full mb-10">
                  ...
                  {pluginData ? (
                    <div className="w-full h-full flex flex-col animate-in fade-in slide-in-from-bottom-8 duration-600">
                      ...
                      <div className="flex-1 min-h-0 bg-white rounded-[3.5rem] border border-slate-100 p-1 shadow-2xl shadow-slate-200/40 overflow-hidden">
                        {pluginData.result.step === 'SUCCESS' ? (
                          <div className="h-full flex flex-col items-center justify-center p-10 text-center animate-in zoom-in">
                             ...
                             <button onClick={() => setPluginData(null)} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] text-xs font-black uppercase tracking-[0.4em] shadow-2xl active:scale-95 transition-all">关闭任务面板</button>
                          </div>
                      ...
                      <button onClick={toggleVoice} className="mt-6 self-center p-6 bg-white shadow-2xl rounded-full border border-slate-100 active:scale-[0.85] transition-all">
                         {isListening ? <X size={24} className="text-rose-500" /> : <Mic size={24} className="text-indigo-600" />}
                      </button>
                    </div>
                  ) : !isListening && !isSpeaking && (
                    <div className="grid grid-cols-2 gap-5 w-full animate-in zoom-in duration-700">
                      <button onClick={() => triggerPluginDirectly('food_delivery')} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col items-center gap-5 active:scale-95 transition-all group hover:border-orange-200">
                        <div className="w-16 h-16 bg-orange-50 text-orange-500 rounded-[2rem] flex items-center justify-center group-hover:bg-orange-500 group-hover:text-white transition-all duration-300 shadow-sm"><Utensils size={32} /></div>
                        <div className="text-center">
                           <span className="text-sm font-black text-slate-800 block mb-1">极速订餐</span>
                           <span className="text-[8px] text-slate-300 font-black uppercase tracking-[0.2em]">画像同步</span>
                        </div>
                      </button>
                      <button onClick={() => triggerPluginDirectly('system_control')} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col items-center gap-5 active:scale-95 transition-all group hover:border-indigo-200">
                        <div className="w-16 h-16 bg-indigo-50 text-indigo-500 rounded-[2rem] flex items-center justify-center group-hover:bg-indigo-500 group-hover:text-white transition-all duration-300 shadow-sm"><Zap size={32} /></div>
                        <div className="text-center">
                           <span className="text-sm font-black text-slate-800 block mb-1">深度加速</span>
                           <span className="text-[8px] text-slate-300 font-black uppercase tracking-[0.2em]">资源释放</span>
                        </div>
                      </button>
                    </div>
                  )}
            ...
            {!pluginData && (
              <div className="shrink-0 space-y-6 pb-10 pt-6">
                <section className="animate-in slide-in-from-bottom-4 duration-500 delay-200">
                   <div className="flex items-center gap-3 mb-5 px-4">
                     ...
                     <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">神经连接日志</h3>
                   </div>
                   <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden p-1.5">
                      {history.length > 0 ? history.map((item, idx) => (
                        <button 
                          key={item.id} 
                          onClick={() => resumeFromHistory(item)}
                          className={`w-full flex items-center gap-5 px-7 py-5 hover:bg-slate-50 transition-colors active:bg-slate-100 ${idx !== 0 ? 'border-t border-slate-50' : ''}`}
                        >
                          <div className={`w-2.5 h-2.5 rounded-full ${item.type === 'user' ? 'bg-indigo-400' : 'bg-orange-400'}`} />
                          <p className="text-xs font-black text-slate-600 truncate flex-1 leading-none text-left">{item.content}</p>
                          <span className="text-[9px] font-black text-slate-300 uppercase shrink-0">{new Date(item.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        </button>
                      )) : <div className="p-8 text-center text-[11px] text-slate-300 italic font-black uppercase tracking-widest">无活动链路</div>}
                   </div>
                </section>

                <button 
                  onClick={() => setShowPanel('profile')}
                  className="w-full bg-slate-900 rounded-[3rem] p-7 shadow-2xl flex items-center gap-6 active:scale-[0.98] transition-all border border-white/5 group"
                >
                   <div className="w-16 h-16 bg-white/10 text-white rounded-[1.8rem] flex items-center justify-center shrink-0 group-hover:bg-indigo-500 transition-colors">
                      <TrendingUp size={32} />
                   </div>
                   <div className="flex-1 text-left min-w-0">
                      <h4 className="text-sm font-black text-white uppercase tracking-[0.2em] mb-1.5">动态画像库</h4>
                      <p className="text-[10px] text-white/40 truncate font-bold">已同步 {profile.orderHistory.length} 行为节点 · 指令画像已生效</p>
                   </div>
      ...
      <footer className="px-10 pb-10 flex justify-between items-center z-10 shrink-0">
        <div className="flex items-center gap-4">
          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-lg shadow-emerald-500/50" />
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">核心状态：稳定</span>
        </div>
        <div className="text-slate-300 text-[10px] font-black uppercase flex items-center gap-2 tracking-[0.2em]">
          <Globe size={14} className="text-indigo-300" /> {llmConfig.provider}
        </div>
      </footer>
    </div>
  );
